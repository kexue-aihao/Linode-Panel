...
- VM 管理：Linode 实例列表、创建、开机、重启、关机、删除、新增公网 IPv4。
- 官方 API：内置 Akamai/Linode API v4 通用控制台，可执行官方文档支持的 `/v4/...` 接口。
...
进入「官方 API」后，可以用默认 Linode 号池 Token 直接调用 Akamai/Linode 官方 API v4：

- 支持 `GET`、`POST`、`PUT`、`PATCH`、`DELETE`。
- API 路径必须以 `/v4/` 开头，例如 `/v4/profile`、`/v4/linode/instances`。
- `GET` 请求可勾选“自动翻页”，面板会聚合分页结果。
- `POST`、`PUT`、`PATCH`、`DELETE` 执行前会二次确认。
- 请求和结果会进入「执行日志」。

该控制台用于覆盖官方 API 支持但暂未做成专用页面的全部功能，例如账号、账单、事件、实例、IP、卷、镜像、StackScript、防火墙、VPC、NodeBalancer、对象存储、LKE、托管数据库、Linode DNS、工单、Beta 等模块。
...
进入「VM 管理」后可以执行实例生命周期操作：

- 创建实例。
- 开机、重启、关机、删除实例。
- 换 IP：面板会调用 Linode 官方 API `POST /v4/networking/ips`，为当前实例新增一个公网 IPv4。

注意：Linode 官方 API 的新增 IPv4 操作不会自动删除旧 IP。面板也不会自动移除旧 IP，避免业务尚未切换时造成失联。建议确认新 IP 可用、DNS 已切换、业务访问正常后，再到 Linode 后台或官方 API 处理旧 IP。

### DDoS 防护说明

已查询 Linode/Akamai 官方 API 文档，目前没有发现可对单台 VM 实例执行“开启 DDoS 防护计划”的实例级 API 开关。因此面板不会提供伪造的 DDoS 开启按钮。

DDoS 防护状态请以 Linode/Akamai 控制台或支持渠道显示的账号/平台防护状态为准。VM 卡片中的「DDoS 说明」只展示该能力边界。
...
如果要使用「换 IP / 新增公网 IPv4」，Token 必须具备管理 Linode 实例和公网 IP 地址的写入权限。通常需要 `linodes:read_write`；如果 Linode Token 创建页面单独显示 IP Addresses / IP 地址权限，也请授予读写。
